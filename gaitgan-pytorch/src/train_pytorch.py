...
from data_set_pytorch import CASIABDataset
...
netd.train()
neta.train()
train_dataset = CASIABDataset(data_dir='/GaitDatasetB_gei/')
...
for iteration in range(450):                          
  for i, data in enumerate(train_loader):
      #print(data[0].shape)
      ass_label, noass_label, img = data[0],data[1],data[2]
      label = th.zeros((img.shape[0], 1), requires_grad=False).to(device)
      ...
      if iteration % 10 == 0:
          ...
          th.save(state, './epoch_%d.t7' % iteration)
